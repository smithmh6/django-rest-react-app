...
class Purchasing extends Component {
  ...
  fetchData = async () => {
    ...
    try {
      const data = await Promise.all([
        RequestService.makeRequest('GET', `${baseUrl}approved-items/`),
  ...
  checkFilterResults = () => {
    this.setState((prevState) => ({
      ...
      errors:
        prevState.purchaseItems.length === 0
  ...
  render() {
    if (!this.state.isFetching) {
      ...
      if (this.state.statusItems.itemStatusChoices)
        purchaseItemFields.get('status').options.choices =
          this.state.statusItems.itemStatusChoices.filter(
            (choice) => !['CREATE', 'PREC', 'RECV'].includes(choice.code)
          );
      ...
      const filterFields = Array.from(purchaseItemFields).filter(([name]) => {
        return ['description', 'vendor', 'username'].includes(name);
      });

      const { errors, showDialog, messageText } = this.state;
